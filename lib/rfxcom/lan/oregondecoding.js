...
class oregondecoding extends libNew {
    ...
    constructor() {
        ...
        //#region choosedevice
        let makeHomeyDriverCompatibleAandPasstoDriver = (filledDevice, result) => {

            // is not a homey capabilty but capabilities with value
            let capability = {}
            let capabilities = []
            filledDevice.data.id = result.id + result.rolling
            ...
            switch (result.id) {
                ...
                case "5d60":
                case "5d53":
                    filledDevice.data.type = `THB`
                    filledDevice.data.houseCode = 19
                    filledDevice.data.unitCode = 19
                    capabilities = Homey.app.rfxcomDeviceTypes[filledDevice.data.type].capabilities
                    capability = Homey.app.rfxcomDeviceTypes[filledDevice.data.type].capability
                    capability.measure_temperature = parseFloat(parseFloat(result.data.temperature).toFixed(2))
                    capability.measure_humidity = parseInt(result.data.humidity)
                    capability.measure_pressure =  parseInt(result.data.pressure)
                    capability.measure_battery =  result.lowbattery  // onoff dim temp etc as json  object of capabilities
                    filledDevice.capabilities = capabilities
                    filledDevice.capability = capability
                    filledDevice.icon = `/icons/${filledDevice.data.type}.svg`
                    filledDevice.name = filledDevice.rx.type + result.id + result.rolling
                ...
                case "e2cf":    // kleine corona THGN228NX
                    filledDevice.data.type = `TH`
                    this.processTEMPHUM(filledDevice, result);
                    break;
                case "2914":  // regenmeter
                    filledDevice.data.type = `R`
                    this.processRAIN(filledDevice, result);
                    break;
                case "ec70":  // UV uv meter corona
                    filledDevice.data.type = `U`
                    this.processUV(filledDevice, result);
                    break;
                case "ec40":  // THN132n only temp c844
                case "c844":  // THN132n only temp c844
                    filledDevice.data.type = `T`
                    this.processTEMP(filledDevice, result);
            }
        ...
        this.processTEMP = (Fdevice, result) => {
            ...
            driverOregon.log('temp device oregon ', filledDevice)
        ...
        this.processTEMPHUM = (Fdevice, result) => {
            ...
            let filledDevice = {
                data: {
                    ...
                    houseCode: 17,  // otherwise no device is found with data
                    unitCode: 17,
                    protocol: Fdevice.data.protocol, //  visonic , x10 , oregon , etc klika elro etc handlers 
                    type: 'TH'// type of device eg visonicdoorsensor      
                },
                driver: 'TEMPHUM',
                name: 'TH' + result.id + result.rolling, // Homey.app.app + device.rx.type
                rx: Fdevice.rx, // index of rxtx where signals are received from for this device
            ...
            driverOregon.log('temphum device oregon ', filledDevice)
            ...
            if (!this.contains(driverTEMPHUM.homeyDevices, filledDevice)) {
                ...
                // driverOregon.log('driver get devices  ', util.inspect(driverTEMPHUM.getDevices()))

                driverOregon.log('instance of Homey.device ', device instanceof Homey.Device)
        ...
        this.processTEMPHUMBAR = (Fdevice, result) => {


            driverOregon.log('  processTEMPHUMBAR(result) enterd  ');
            driverOregon.log('temp ', result.data.temperature, typeof result.data.temperature)
            driverOregon.log('pressure ', result.data.pressure, typeof result.data.pressure)
            ...
            driverOregon.log(`filled device ${util.inspect(filledDevice)}`);
        ...
        this.processOregonDevice = (filledDevice ) =>
        {
            ...
            //    tx: [], // index of rxtx where franes are send to for this device 
            //    name: Fdevice.rx.type + result.id + result.rolling,            //   rxtx type and old id 
            //    capabilities: capabilities = Homey.app.rfxcomDeviceTypes[Fdevice.data.type].capabilities,
            //    capability: {
            //        measure_temperature: parseFloat(parseFloat(result.data.temperature).toFixed(2)),
            //        measure_humidity: parseInt(result.data.humidity),
            //        measure_pressure: parseInt(result.data.pressure),
        ...
        this.processOregon = (Fdevice, result) => {
            driverOregon.log('  processOregon(result) enterd  ');
            driverOregon.log('temp ', result.data.temperature, typeof result.data.temperature)
            driverOregon.log('pressure ', result.data.pressure, typeof result.data.pressure)
