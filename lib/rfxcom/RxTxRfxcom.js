...
class RfxcomRxTx extends newLib {
    ...
    constructor(RxTx) {
        ...
        this.clientRx = new socketClient(this.capability, this.ip, this.rx, this.type, this.processReceivedData)

        // lan type with tx
        if(this.type == 'TxLan' || this.type == 'RxTxLan') {
            this.capability = "tx"
            this.clientTx = new socketClient( this.capability, this.ip, this.tx, this.type, this.processReceivedData)
        }
