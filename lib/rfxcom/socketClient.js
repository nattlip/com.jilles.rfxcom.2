...
const easywavedecoding = require('./easywave/easywaveDecoding.js');  // rfxtrx
...
class socketClient extends newLib {
    ...
    constructor(capability, ip, port, type, processReceivedData, sendData) {
        ...
        this.pair = false; // for pairing Ew
        const eol = ' \n';
        const eolf = ' \n\r';
        let previousDataStr = ``;
        ...
        this.openClient = (capability, ip, port, type) => {
            ...
            if (capability === 'rx') {
                ...
                if (this.client === undefined) {
                    this.lib.log(`new rx socket  instantiated`);
                ...
                if (this.capability === 'rx') {
                    ...
                    this.client.on('connect', () => {
                        ...
                        if (this.type === 'RxLan' || this.type === 'RxTxLan') {
                            ...
                            this.lib.log(`type rx or rxtxlan send modecommand`);
                            ...
                            this.lib.log(`type must be rfxtrx RfxCom  type  ${type}  ip ${ip}  port ${port}`);
                            //this.client.setEncoding('hex')
                            this.sendStartSequenceTrx(this.client);
                            //  this.client.setEncoding('hex')
                            this.client.setTimeout(0);
                        }
                        else if (this.type === 'EldatPi') {  // encoding is utf8
                            this.lib.log(`type must be rfxtrx RfxCom  type  ${type}  ip ${ip}  port ${port}`);
                            //this.client.setEncoding('hex')
                            //  this.client.setEncoding('hex')
                            this.client.setTimeout(0);
                ...
                this.client.on('data', (dataStr) => {
                    ...
                    if (this.processReceivedData) {
                            ...
                            if (dataStr === `40` || dataStr === `2c`) {
                                ...
                                if (dataStr.length === 2) {
                                    ...
                                    while (newDataString.length > 0) {
                                        ...
                                        // this.lib.log(' firstByteBin ', firstByteBin)
                                        ...
                                        // this.lib.log('length ', length)
                                        // this.lib.log('parseInt(length) ', parseInt(length))
                                        // this.lib.log(' 2 * parseInt(length) + 2 ', 2 * parseInt(length) + 2)

                                        let message = '';
                                        if (Number.isInteger(parseInt(length))) {

                                            if (firstbyte === '5c') { message = newDataString }
                                            else { message = newDataString.slice(0, 2 * parseInt(length) + 2); }
                        ...
                        if (this.type === 'RfxTrx') {
                            ...
                            this.lib.log('rfxtrx received Buffer from   ', buf);
                        ...
                        if (this.type === 'EldatPi') {


                            // dataStr is received data is ascci string 
                            dataStr;   // encoding is default  === utf8
                            ...
                            easywavedecoding.receiveSerialData(this.pair, dataStr, rx);
        ...
        this.sendStartSequenceTrx = (client) => {
            ...
            setTimeout(() => {
                ...
                client.write(bufferB, (err, results) => {
                    ...
                    this.lib.log(' this.sendStartSequenceTrx error write B ', err);
                ...
                setTimeout(() => {
                    ...
                    client.write(bufferB2, (err, results) => {
                        ...
                        this.lib.log('this.sendStartSequenceTrx B2 error write  error write ', err);
                        this.lib.log('error result write ', results);
        ...
        this.sendCommandEldatPi = (sendData) => {
            ...
            if (this.sendData) {
                ...
                this.lib.log(` sendDataEldatP  capability ${capability} ip ${ip} port ${port} type ${type}`);
                this.pair = sendData.pair;
                let stringCommand = `TXP,${sendData.channel},${command}\n`;

                this.lib.log(`sendDataEldatPi sends command ${stringCommand}`);

                this.client.write(stringCommand, (err, results) => {
        ...
        let lanDecodeData = (hexStr) => {
            ...
            let filledDevice =
            {
                data: {
                    ...
                    type: null // type of device eg visonicdoorsensor ot thb or tb
                },
                driver: null,
            ...
            if (firstbyte === '50' || firstbyte === '60' || firstbyte === '78' || firstbyte === '5c') {
                filledDevice.data.protocol = 'oregon';
                filledDevice.driver = `oregon`;
